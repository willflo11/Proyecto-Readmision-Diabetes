...
Utilicé un [dataset público de 100,000 pacientes del UCI](https://archive.ics.uci.edu/ml/datasets/diabetes+130-us+hospitals+for+years+1999-2008), que es conocido por sus desafíos de limpieza y desbalanceo de clases.

## El Desafío: Datos Sucios y Desbalanceados

Mi primer objetivo fue replicar un escenario real de ciencia de datos. El dataset crudo presentaba varios retos significativos:
...
* Identifiqué y convertí los nulos ocultos (`?`) a `NaN` para poder cuantificarlos.
...
* **Decisión Clínica:** Analicé los nulos en `A1Cresult` y `max_glu_serum`. Determiné que estos `NaN` (más de 174,000) no eran "datos faltantes", sino que representaban **"Prueba No Realizada"**. En lugar de eliminar estos datos, creé una nueva categoría, `'None'`, tratando la "no-prueba" como una característica predictiva en sí misma.
...
*(Puedes insertar aquí tu primera Matriz de Confusión subiendo la imagen al repositorio y enlazándola)*
...
* **Conclusión:** En un escenario hospitalario real, un modelo que "captura" al 56% de los pacientes de alto riesgo (para darles seguimiento) es **infinitamente más valioso** que uno que captura al 0%.
